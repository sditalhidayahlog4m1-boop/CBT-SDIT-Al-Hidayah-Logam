...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const teacherNotLoggedInCount = teacherAccounts.filter((u) => !u.isLoggedIn).length;
  const teacherOnlineCount = teacherAccounts.filter((u) => u.isOnline).length;
  const teacherJustLoginCount = teacherAccounts.filter((u) => u.isLoggedIn && u.examCount === 0 && u.gameCount === 0 && !u.currentActivity.toLowerCase().includes('ujian')).length;
  ...
  const studentNotLoggedInCount = studentAccounts.filter((u) => !u.isLoggedIn).length;
  const studentOnlineCount = studentAccounts.filter((u) => u.isOnline).length;
  const studentExamCount = studentAccounts.filter((u) => u.examCount > 0 || u.currentActivity.toLowerCase().includes('ujian')).length;
  const studentJustLoginCount = studentAccounts.filter((u) => u.isLoggedIn && u.examCount === 0 && u.gameCount === 0 && !u.currentActivity.toLowerCase().includes('ujian')).length;

  const adminAccounts = userAccountList.filter((u) => u.role === 'admin' || u.role === 'umum');
  const adminOnlineCount = adminAccounts.filter((u) => u.isOnline).length;
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="bg-[#0f172a] rounded-2xl border border-slate-800 shadow-xl overflow-hidden">
        ...
        <div className="p-5 sm:p-6 border-b border-slate-800/80 bg-gradient-to-r from-slate-900 via-slate-900/80 to-slate-950">
          ...
          {/* Quick Metrics Bar: Ringkasan Utama */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-5 pt-4 border-t border-slate-800/70">
            ...
            <div className="bg-slate-950/70 rounded-xl p-3 border border-slate-800 flex items-center justify-between">
              <div>
                <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">Total Akun CBT</span>
                <span className="text-xl font-black text-slate-100">{totalAccounts}</span>
                <div className="text-[11px] text-slate-400 font-medium mt-0.5">
                  <span className="text-blue-400 font-bold">{teacherAccounts.length} Guru</span> • <span className="text-purple-400 font-bold">{studentAccounts.length} Siswa</span>
                </div>
              </div>
            ...
            <div className="bg-slate-950/70 rounded-xl p-3 border border-emerald-500/30 flex items-center justify-between shadow-sm shadow-emerald-950/40">
              <div>
                ...
                <div className="text-[11px] text-emerald-400/90 font-medium mt-0.5">
                  <span>{teacherOnlineCount} Guru</span> • <span>{studentOnlineCount} Siswa</span>
                </div>
              </div>
            ...
            {/* Siswa Sedang / Selesai Ujian */}
            <div className="bg-slate-950/70 rounded-xl p-3 border border-slate-800 flex items-center justify-between">
              <div>
                <span className="text-[11px] font-bold text-indigo-400 uppercase tracking-wider block">Siswa Ujian CBT</span>
                <div className="flex items-baseline gap-1.5 mt-0.5">
                  <span className="text-xl font-black text-indigo-300">{studentExamCount}</span>
                  <span className="text-xs text-slate-400 font-semibold">Siswa</span>
                </div>
                <div className="text-[11px] text-slate-400 font-medium mt-0.5">
                  <span>Sedang / Selesai Ujian</span>
                </div>
              </div>
            ...
            {/* Total Belum Login */}
            <div className="bg-slate-950/70 rounded-xl p-3 border border-rose-500/30 flex items-center justify-between shadow-sm shadow-rose-950/30">
              <div>
                <span className="text-[11px] font-bold text-rose-400 uppercase tracking-wider block">Total Belum Login</span>
                <div className="flex items-baseline gap-1.5 mt-0.5">
                ...
                <div className="text-[11px] text-rose-300 font-semibold mt-0.5">
                  <span>{teacherNotLoggedInCount} Guru</span> • <span>{studentNotLoggedInCount} Siswa</span>
                </div>
              </div>
          ...
          {/* DUA PANEL KHUSUS: PEMISAHAN STATUS LOGIN GURU & SISWA */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
            {/* PANEL GURU */}
            <div className="bg-gradient-to-br from-blue-950/30 via-slate-900/90 to-slate-950 rounded-2xl p-4 border border-blue-500/30 shadow-md flex flex-col justify-between">
              <div>
                <div className="flex items-center justify-between pb-3 border-b border-blue-500/20">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h4 className="text-sm font-black text-blue-200 tracking-tight">Status Login Dewan Guru</h4>
                      <span className="text-[11px] text-blue-400/80 font-medium">Total: {teacherAccounts.length} Guru terdaftar</span>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-2.5 my-3">
                  {/* Guru Baru / Sudah Login */}
                  <div className="bg-slate-950/80 p-2.5 rounded-xl border border-slate-800">
                    <div className="flex items-center gap-1.5 text-amber-400 text-[11px] font-bold">
                      <Clock className="w-3.5 h-3.5 text-amber-400" />
                      <span>Baru / Sudah Login</span>
                    </div>
                    <div className="text-lg font-black text-amber-300 mt-1">
                      {teacherLoggedInCount} <span className="text-xs font-normal text-slate-400">Guru</span>
                    </div>
                    <p className="text-[10px] text-slate-400 mt-0.5">{teacherJustLoginCount} baru login • {teacherOnlineCount} online</p>
                  </div>

                  {/* Guru Belum Login */}
                  <div className={`p-2.5 rounded-xl border shadow-sm ${
                    teacherNotLoggedInCount > 0
                    ...
                    <div className="text-lg font-black text-rose-400 mt-1">
                      {teacherNotLoggedInCount} <span className="text-xs font-normal text-slate-400">Guru</span>
                    </div>
                    <p className="text-[10px] text-rose-400/90 font-semibold mt-0.5">
                      {teacherNotLoggedInCount > 0 ? '⚠️ Belum pernah masuk' : 'Semua guru sudah login'}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2 pt-1 border-t border-slate-800/60">
                <button
                  ...
                  className={`flex-1 min-w-[140px] px-2.5 py-1.5 rounded-lg text-xs font-bold flex items-center justify-center gap-1.5 transition-all cursor-pointer border ${
                    activeRoleFilter === 'guru' && activeStatusFilter === 'not_logged'
                      ? 'bg-rose-600 text-white border-rose-500 shadow-sm'
                      : 'bg-rose-500/15 hover:bg-rose-500/25 text-rose-300 border-rose-500/30'
                  }`}
                  title="Klik untuk mendeteksi siapa saja Guru yang Belum Login"
                >
                  <XCircle className="w-3.5 h-3.5" />
                  <span>Lihat Guru Belum Login ({teacherNotLoggedInCount})</span>
                </button>
                <button
                  onClick={() => {
                    setActiveRoleFilter('guru');
                  ...
                  className={`px-2.5 py-1.5 rounded-lg text-xs font-bold flex items-center justify-center gap-1.5 transition-all cursor-pointer border ${
                    activeRoleFilter === 'guru' && activeStatusFilter === 'just_logged'
                      ? 'bg-amber-600 text-white border-amber-500 shadow-sm'
                      : 'bg-amber-500/15 hover:bg-amber-500/25 text-amber-300 border-amber-500/30'
                  }`}
                  title="Klik untuk melihat Guru yang Baru Login"
                >
                  <Clock className="w-3.5 h-3.5" />
                  <span>Baru Login ({teacherJustLoginCount})</span>
                </button>
                <button
                  onClick={() => {
                    setActiveRoleFilter('guru');
                  ...
                  className="px-2.5 py-1.5 rounded-lg text-xs font-bold bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 transition-all cursor-pointer"
                >
                  Semua ({teacherAccounts.length})
                </button>
              </div>
            </div>

            {/* PANEL SISWA */}
            <div className="bg-gradient-to-br from-purple-950/30 via-slate-900/90 to-slate-950 rounded-2xl p-4 border border-purple-500/30 shadow-md flex flex-col justify-between">
              <div>
                <div className="flex items-center justify-between pb-3 border-b border-purple-500/20">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h4 className="text-sm font-black text-purple-200 tracking-tight">Status Login Siswa / Peserta</h4>
                      <span className="text-[11px] text-purple-400/80 font-medium">Total: {studentAccounts.length} Siswa terdaftar</span>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-2 my-3">
                  {/* Siswa Baru Login (Belum Ujian) */}
                  <div className="bg-slate-950/80 p-2.5 rounded-xl border border-slate-800">
                    <div className="flex items-center gap-1 text-amber-400 text-[10px] font-bold">
                      <Clock className="w-3 h-3 text-amber-400 shrink-0" />
                      <span className="truncate">Baru Login</span>
                    </div>
                    <div className="text-lg font-black text-amber-300 mt-1">
                      {studentJustLoginCount} <span className="text-xs font-normal text-slate-400">Siswa</span>
                    </div>
                    <p className="text-[10px] text-slate-400 mt-0.5 truncate">Belum mulai ujian</p>
                  </div>
                  ...
                  <div className="bg-slate-950/80 p-2.5 rounded-xl border border-slate-800">
                    <div className="flex items-center gap-1 text-indigo-400 text-[10px] font-bold">
                      ...
                      <span className="truncate">Ujian CBT</span>
                    </div>
                    <div className="text-lg font-black text-indigo-300 mt-1">
                      {studentExamCount} <span className="text-xs font-normal text-slate-400">Siswa</span>
                    </div>
                    <p className="text-[10px] text-slate-400 mt-0.5 truncate">Sedang/Sudah ujian</p>
                  </div>

                  {/* Siswa Belum Login */}
                  <div className={`p-2.5 rounded-xl border shadow-sm ${
                    studentNotLoggedInCount > 0
                      ? 'bg-rose-950/40 border-rose-500/40 shadow-rose-950/30'
                      : 'bg-slate-950/80 border-slate-800'
                  }`}>
                    <div className="flex items-center gap-1 text-rose-400 text-[10px] font-bold">
                      <XCircle className="w-3 h-3 text-rose-400 shrink-0" />
                      <span className="truncate">Belum Login</span>
                    </div>
                    <div className="text-lg font-black text-rose-400 mt-1">
                      {studentNotLoggedInCount} <span className="text-xs font-normal text-slate-400">Siswa</span>
                    </div>
                    <p className="text-[10px] text-rose-400/90 font-semibold mt-0.5 truncate">
                      {studentNotLoggedInCount > 0 ? '⚠️ Belum masuk' : 'Semua sudah login'}
                    </p>
                  </div>
                </div>
              </div>

              {/* Action Buttons Siswa */}
              <div className="flex flex-wrap items-center gap-2 pt-1 border-t border-slate-800/60">
                <button
                  onClick={() => {
                  ...
                  className={`flex-1 min-w-[140px] px-2.5 py-1.5 rounded-lg text-xs font-bold flex items-center justify-center gap-1.5 transition-all cursor-pointer border ${
                    activeRoleFilter === 'siswa' && activeStatusFilter === 'not_logged'
                      ? 'bg-rose-600 text-white border-rose-500 shadow-sm'
                      : 'bg-rose-500/15 hover:bg-rose-500/25 text-rose-300 border-rose-500/30'
                  }`}
                  title="Klik untuk mendeteksi siapa saja Siswa yang Belum Login"
                >
                  <XCircle className="w-3.5 h-3.5" />
                  <span>Lihat Siswa Belum Login ({studentNotLoggedInCount})</span>
                </button>
                <button
                  onClick={() => {
                    setActiveRoleFilter('siswa');
                  ...
                  className={`px-2.5 py-1.5 rounded-lg text-xs font-bold flex items-center justify-center gap-1.5 transition-all cursor-pointer border ${
                    activeRoleFilter === 'siswa' && activeStatusFilter === 'just_logged'
                      ? 'bg-amber-600 text-white border-amber-500 shadow-sm'
                      : 'bg-amber-500/15 hover:bg-amber-500/25 text-amber-300 border-amber-500/30'
                  }`}
                  title="Klik untuk melihat Siswa yang Baru Login"
                >
                  <Clock className="w-3.5 h-3.5" />
                  <span>Baru Login ({studentJustLoginCount})</span>
                </button>
                <button
                  onClick={() => {
                    setActiveRoleFilter('siswa');
        ...
        <div className="p-4 sm:p-5 bg-slate-950/40 border-b border-slate-800/80 flex flex-col md:flex-row md:items-center justify-between gap-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {(activeRoleFilter !== 'all' || activeStatusFilter !== 'all' || selectedClass !== 'all' || searchQuery.trim()) && (
              <button
                onClick={() => {
                ...
                className="px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-bold border border-slate-700 transition-colors flex items-center gap-1.5 cursor-pointer shadow-sm"
                title="Kembalikan dan tampilkan semua data"
              >
                <XCircle className="w-3.5 h-3.5 text-rose-400" />
        ...
        {/* Banner Notifikasi Filter Deteksi Belum Login / Baru Login */}
        {activeStatusFilter === 'not_logged' && (
          <div className="mx-4 sm:mx-6 mt-4 p-3.5 bg-rose-950/40 border border-rose-500/40 rounded-xl flex items-center justify-between gap-3 text-xs shadow-sm shadow-rose-950/20">
              ...
              <div>
                <span className="font-bold text-rose-300">
                  {activeRoleFilter === 'guru'
                    ? `Terdeteksi ${filteredUsers.length} Guru Belum Login:`
                    : activeRoleFilter === 'siswa'
                    ? `Terdeteksi ${filteredUsers.length} Siswa Belum Login:`
                    : `Terdeteksi ${filteredUsers.length} Akun Belum Login (${teacherNotLoggedInCount} Guru, ${studentNotLoggedInCount} Siswa):`}
                </span>{' '}
                <span className="text-slate-300">
                  Daftar akun di bawah belum pernah login/masuk ke sistem CBT.
                </span>
              </div>
            </div>
            <button
              ...
              className="px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold border border-slate-700 text-xs shrink-0 cursor-pointer"
            >
              Reset Filter
            </button>
          </div>
        ...
        {activeStatusFilter === 'just_logged' && (
          <div className="mx-4 sm:mx-6 mt-4 p-3.5 bg-amber-950/30 border border-amber-500/40 rounded-xl flex items-center justify-between gap-3 text-xs shadow-sm shadow-amber-950/20">
            <div className="flex items-center gap-2.5 text-amber-200">
              ...
              <div>
                <span className="font-bold text-amber-300">
                  {activeRoleFilter === 'guru'
                    ? `Terdeteksi ${filteredUsers.length} Guru Baru Saja Login:`
                    : activeRoleFilter === 'siswa'
                    ? `Terdeteksi ${filteredUsers.length} Siswa Baru Login (Belum Ujian):`
                    : `Terdeteksi ${filteredUsers.length} Akun Baru Login (${teacherJustLoginCount} Guru, ${studentJustLoginCount} Siswa):`}
                </span>{' '}
                <span className="text-slate-300">
                  {activeRoleFilter === 'siswa'
                    ? 'Siswa sudah berhasil login ke akun, namun belum memulai pengerjaan paket soal ujian CBT.'
                    : 'Pengguna sudah masuk ke aplikasi CBT.'}
                </span>
              </div>
            </div>
            <button
                ...
                <tr
                  ...
                  className={`hover:bg-slate-800/40 transition-colors ${
                    user.isOnline
                      ...
                      : !user.isLoggedIn && user.role === 'siswa'
                      ? 'bg-rose-950/10'
                      : user.isLoggedIn && user.examCount === 0 && user.gameCount === 0 && !user.currentActivity.toLowerCase().includes('ujian')
                      ? 'bg-amber-950/10'
                      : user.isLoggedIn
                      ? 'bg-slate-900/20'
                      : 'bg-transparent'
                  ...
                  <td className="p-3.5 text-center">
                    {user.isOnline ? (
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 font-black text-[11px] shadow-sm shadow-emerald-950">
                        ...
                        <span>ONLINE ({user.role === 'guru' ? 'GURU' : user.role === 'siswa' ? 'SISWA' : 'ADMIN'})</span>
                      </span>
                    ) : user.isIdle ? (
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amber-500/15 text-amber-300 border border-amber-500/30 font-bold text-[11px]">
                        ...
                        <span>IDLE ({user.role === 'guru' ? 'GURU' : 'SISWA'})</span>
                      </span>
                    ) : user.isLoggedIn ? (
                      user.examCount === 0 && user.gameCount === 0 && !user.currentActivity.toLowerCase().includes('ujian') ? (
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amber-500/15 text-amber-300 border border-amber-500/30 font-bold text-[11px]">
                          <Clock className="w-3 h-3 text-amber-400" />
                          <span>BARU LOGIN ({user.role === 'guru' ? 'GURU' : 'SISWA'})</span>
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-slate-800/90 text-slate-300 border border-slate-700 font-semibold text-[11px]">
                          ...
                          <span>OFFLINE ({user.role === 'guru' ? 'GURU' : 'SISWA'})</span>
                        </span>
                      )
                    ) : (
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full font-bold text-[11px] border ${
                        ...
                        <span>BELUM ({user.role === 'guru' ? 'GURU' : 'SISWA'})</span>
                      </span>
                    )}
                  </td>
