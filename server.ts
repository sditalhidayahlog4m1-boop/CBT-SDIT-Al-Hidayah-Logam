...
async function startServer() {
  ...
  function findQuranSurah(topicText: string) {
    ...
    // Default fallback to Al-Ikhlas if Islamic but not specifically named
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (isIslamic && activeSurah) {
        ...
        if (gameType === "sambung-ayat") {
          ...
          // Default pilihan-ganda / ular-tangga / memory-card
          items.push({
            id: itemId,
            category: gameType,
            subject_name: mapelName,
        ...
        if (gameType === "tebak-audio") {
          ...
          // General subjects other game types
...
4. PEMBAHASAN: Sertakan uraian penjelasan singkat, mendidik, dan informatif pada field "explanation".`;
  ...
  app.post("/api/generate-game", async (req, res) => {
    try {
      ...
      if (!apiKey) {
        console.warn("GEMINI_API_KEY tidak ada. Menggunakan generator cerdas internal untuk game.");
